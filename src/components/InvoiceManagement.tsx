...
export function InvoiceManagement() {
  ...
  const [procedures, setProcedures] = useState([
    { id: 1, description: 'General Consultation', amount: 150.00 }
  ]);
  const [newProcedure, setNewProcedure] = useState({ description: '', amount: '' });
          ...
          <CardContent className="p-8">
            <div className="space-y-8">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-3xl font-bold text-slate-900 mb-2">DigiClinic</h2>
                  <p className="text-slate-600">Healthcare Management</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-slate-600 mt-2">Invoice: {selectedInvoice.id}</p>
                  <p className="text-sm text-slate-600">Date: {selectedInvoice.date}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <p className="text-slate-700">{selectedInvoice.patientName}</p>
                  <p className="text-sm text-slate-600">Patient ID: P001</p>
                </div>
                <div className="text-right">
                  <h3 className="font-semibold text-slate-900 mb-2">Amount Due:</h3>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-slate-900">Procedures & Services</h3>
                  {isEditing && (
                  ...
                  <TableBody>
                    {procedures.map((procedure) => (
                      <TableRow key={procedure.id}>
                        ...
                        <TableCell className="text-right">${procedure.amount.toFixed(2)}</TableCell>
                        {isEditing && (
                      ...
                      <TableRow className="bg-slate-50">
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) => setNewProcedure({ ...newProcedure, amount: e.target.value })}
                    ...
                    <TableRow className="border-t-2">
                      ...
                      <TableCell className="text-right font-semibold text-lg">${getTotalAmount().toFixed(2)}</TableCell>
              ...
              <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-slate-600">
                  Please make payment within 30 days. Thank you for choosing DigiClinic.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      {filteredInvoices.length === 0 && (
        <Card className="border-slate-200">
          <CardContent className="py-12 text-center">
            ...
            <h3 className="font-semibold text-slate-900 mb-2">No invoices found</h3>
            <p className="text-slate-600">
              {searchTerm ? `No invoices match "${searchTerm}"` : "No invoices available"}
            </p>
          </CardContent>
        </Card>
      )}
